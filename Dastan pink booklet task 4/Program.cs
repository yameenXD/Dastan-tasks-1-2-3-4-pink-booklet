...
class Dastan
{
    ...
    private bool AwardWafr(int abundance) // private metgod in the Dastan class called AwardWafr. A parameter abundance declared as an integer.
    {
        abundance = 25; // it is equal to 25
        if (abundance == 25) // if their abundance is 25 it will return true
        {
            return true;
        }
...
class Player
{
    ...
    private int AwardWafr;
    private MoveOptionQueue Queue = new MoveOptionQueue();

   // set the value of the WafrAwarded attribute
    public int GetWafrAwarded() 
    {
        return AwardWafr;
    }
    //get the value of the WafrAwarded attribute
    public void SetWafrAwarded(int value) 
    {
        AwardWafr = value;
    }
